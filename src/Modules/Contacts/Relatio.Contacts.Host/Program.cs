...
try
{
    ...
    builder.Configuration
        ...
        .AddConsul("relatio/contacts", options =>
        {
            options.ConsulConfigurationOptions = c => c.Address = new Uri(consulUrl);
            options.Optional = true;
            options.ReloadOnChange = true;
    ...
    Log.Information("Contacts service - starting...");
    ...
    Log.Information("Contacts service - ready");
    app.Run();
}
catch (Exception ex)
{
    Log.Fatal(ex, "Contacts service - startup failed");
    throw;
}
finally
{
